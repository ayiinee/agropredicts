...
export default function PestDetectionResultsModal({
  ...
  const isCritical = topResult && (topResult.severity === "tinggi" || topResult.probability >= 80);
        ...
        <div className="space-y-4">
          ...
          {!error && results.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <h3 className="text-lg font-semibold mb-2">Tidak Ada Hama Terdeteksi</h3>
              <p className="text-sm">
                Coba ambil foto dengan pencahayaan yang lebih baik atau fokus pada area yang dicurigai.
              </p>
            </div>
          )}
            ...
            <>
              ...
              {isCritical && (
                <Alert className="border-red-300 bg-red-50">
                  ...
                  <AlertTitle className="text-red-800">Peringatan Kritis!</AlertTitle>
                  <AlertDescription className="text-red-700">
                    Hama dengan tingkat keparahan tinggi terdeteksi. Segera ambil tindakan untuk mencegah penyebaran.
                  </AlertDescription>
                </Alert>
              )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold">
                          {topResult.probability}% {topResult.name} terdeteksi
                        </h3>
                  ...
                  {topResult.boundingBox && (
                    <div className="text-xs text-muted-foreground mb-3">
                      <div className="flex items-center gap-1">
                        ...
                        Lokasi: ({Math.round(topResult.boundingBox.x)}, {Math.round(topResult.boundingBox.y)})
                      </div>
                      <div className="flex items-center gap-1">
                        <Info className="h-3 w-3" />
                        Ukuran: {Math.round(topResult.boundingBox.width)}×{Math.round(topResult.boundingBox.height)} px
                      </div>
                    </div>
                  )}
